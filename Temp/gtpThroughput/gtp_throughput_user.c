...
static inline int bpf_lookup_percpu_elem_syscall(int fd, const void *key, void *value) {
    union bpf_attr attr = {
        .map_fd = fd,
        .key = (__u64)(uintptr_t)key,
        .value = (__u64)(uintptr_t)value,
    };
...
int main(int argc, char **argv) {
    ...
    int n_cpus = get_num_possible_cpus();

    while (!exiting) {
        sleep(1);
        __u32 key = 0, next_key;
        struct teid_stats_t *cpu_stats = calloc(n_cpus, sizeof(struct teid_stats_t));
        if (!cpu_stats) {
            perror("calloc");
            break;
        }
        ...
        while (bpf_map_get_next_key(stats_fd, &key, &next_key) == 0) {
            if (bpf_lookup_percpu_elem_syscall(stats_fd, &next_key, cpu_stats) == 0) {
                struct teid_stats_t agg = {0};
                for (int i = 0; i < n_cpus; i++) {
                    agg.packet_count += cpu_stats[i].packet_count;
                    agg.byte_count += cpu_stats[i].byte_count;
                }
                ...
                __u64 delta_bytes = agg.byte_count - entry->last_bytes;
                __u64 bps = delta_bytes * 8;
                entry->last_bytes = agg.byte_count;

                char msg[512];
                snprintf(msg, sizeof(msg),
                         "{\"teid\": %u, \"packets\": %llu, \"bytes\": %llu, \"bitrate\": %llu, \"slice\": %u}",
                         next_key, agg.packet_count, agg.byte_count, bps, slice_id);
